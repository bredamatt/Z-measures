...
Z7 = function(dataset, k){
  ...
  #Step 1 + 2; models
  models = rollapply(df, width = k, FUN = function(x) 
  coef(lm(r~t, data = as.data.frame(x))), by.column = FALSE, align ="right")
  
  #Step 1 + 2; residuals
  res = rollapply(df, width = k, FUN = function(z) 
  residuals(lm(r~t, data = as.data.frame(z))), by.column = FALSE, align ="right")
