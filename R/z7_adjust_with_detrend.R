...
z7_adjusted_detrend = function(data, width, threshold){
  require(zoo)
  ...
  #For tau4 and S2
  if(abs(tau4*prediction) < e){
    z7_tau4_2 = -(fEA + prediction) / S2
  }
  else 
    z7_tau4_2 = -(fEA + prediction) / (tau4*prediction)
}

test4 = z7_adjusted_detrend(df,3,0.001)
test4
